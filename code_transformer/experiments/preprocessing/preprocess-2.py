...
from code_transformer.preprocessing.nlp.vocab import (
    WordCounter,
    CodeSummarizationVocabularyTransformer,
...
class Preprocess2Container:
    ...
    def _init_binning(
        self, num_bins, n_fixed_bins, exponential_binning, exponential_binning_growth_factor, bin_padding
    ):
    ...
    def _setup_data_managers(self):
        if self.use_multi_language:
            ...
            self.input_data_managers = [
                CTBufferedDataManager(self.input_path, l, self.partition) for l in self.languages
            ]
            word_counters = zip(
            ...
            word_counters = [
                ...
                if i < 3 or not self.separate_label_vocabulary
                else self._combine_counters(word_counter, self.min_vocabulary_frequency_labels)
    ...
    def _setup_vocabularies(self):
        if self.partition == "train":
            ...
            if self.separate_label_vocabulary:
                ...
                ) = self.output_data_manager.load_vocabularies()
            else:
                (
                    self.word_vocab,
                    self.token_type_vocab,
    ...
    def _setup_vocabulary_transformer(self):
        if self.separate_label_vocabulary:
            self.vocabulary_transformer = CodeSummarizationVocabularyTransformer(
                self.word_vocab, self.token_type_vocab, self.node_type_vocab, self.word_vocab_labels
            )
        else:
    ...
    def _setup_distances_transformer(self):
        ...
        if self.exponential_binning:
            db = DistanceBinning(
                self.num_bins, self.n_fixed_bins, ExponentialBinning(self.exponential_binning_growth_factor)
            )
        else:
    ...
    def _combine_counters(self, counters, min_vocab_frequency):
        ...
        df = pd.DataFrame.from_dict({language: counter.words for language, counter in zip(self.languages, counters)})
        df = df.fillna(0)
    ...
    @staticmethod
    def _process_batch(
        x, vocabulary_transformer, distances_transformer, use_multi_language, max_num_tokens, remove_punctuation
    ):
        i, batch = x
        try:
            output = []
            for sample in batch:
                ...
                if use_multi_language:
                    sample = CTStage2MultiLanguageSample(
                        ...
                        sample.docstring,
                        sample_language,
                        encoded_func_name=sample.encoded_func_name if hasattr(sample, "encoded_func_name") else None,
                    )
    ...
    def run(self):
        ...
        with parallel_backend("loky") as parallel_config:
            ...
            while True:
                dataset_slice = itertools.islice(
                    batched_samples_generator, int(self.dataset_slice_size / self.batch_size)
                )
                with Timing() as t:
        ...
        if self.separate_label_vocabulary:
            self.output_data_manager.save_vocabularies(
                self.word_vocab, self.token_type_vocab, self.node_type_vocab, self.word_vocab_labels
            )
        else:
