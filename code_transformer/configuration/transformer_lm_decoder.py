...
class TransformerLMDecoderConfig(ModelConfiguration):
    def __init__(
        ...
        self.pointer_attention_type = (
            pointer_attention_type
            if isinstance(pointer_attention_type, AttentionType)
            else AttentionType(pointer_attention_type)
        )
        ...
        assert not (
            use_pointer_query_self_attention and use_pointer_query_linear
        ), "Cannot set both query linear and query self attention"
