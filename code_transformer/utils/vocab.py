...
def decode_tokens(
    tokens: torch.Tensor, data_manager: CTPreprocessedDataManager = None, word_vocab=None, config=None
) -> List[List[str]]:
    assert (
        data_manager is not None or word_vocab is not None and config is not None
    ), "Either data_manager or word_vocab and config have to be provided"
