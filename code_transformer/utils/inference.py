...
def make_batch_from_sample(stage2_sample: CTStage2Sample, model_config, model_type):
    ...
    data_manager = CTPreprocessedDataManager(
        DATA_PATH_STAGE_2, model_config["data_setup"]["language"], partition="train", shuffle=True
    )
...
def decode_predicted_tokens(tokens, batch, data_manager):
    ...
    predicted_method_name = [
        reverse_lookup(sub_token_id, batch, method_name_vocab)
        for sub_token_id in prediction
        if sub_token_id.item() != 3 and sub_token_id.item() != 0
    ]
...
def predict_method_name(model, model_config, code_snippet: str, method_name_place_holder="f"):
    ...
    lexer_language = (
        "java"
        if language in {"java-small", "java-medium", "java-large", "java-small-pretrain", "java-pretrain"}
        else language
    )
