...
class CodeTransformerModelManager(ModelManager):
    def __init__(self):
        super(CodeTransformerModelManager, self).__init__(MODELS_SAVE_PATH, "ct_code_summarization", "CT")
    ...
    def load_model(self, run_id, snapshot_iteration, gpu=True):
        ...
        try:
            ...
            model.add_module(
                ...
                MultiheadAttention(
                    model.d_model, decoder_config["decoder_nhead"], dropout=decoder_config["decoder_dropout"]
                ),
            )
            try:
                model.load_state_dict(model_params)
