...
from code_transformer.preprocessing.pipeline.filter import (
    CodePreprocessor,
    CommentsRemover,
    EmptyLinesRemover,
    StringMasker,
...
class CTStage1Preprocessor:
    def __init__(self, language, allow_empty_methods=False, use_tokens_limiter=True, max_num_tokens=MAX_NUM_TOKENS):
        ...
        indent_transformer = IndentTransformer(
            INDENT_TOKEN, DEDENT_TOKEN, fix_first_indent=True, allow_empty_methods=allow_empty_methods
        )
        ...
        self.code_preprocessor = CodePreprocessor(
            tokenizer,
            [comments_remover, empty_lines_remover],
            [indent_transformer, whitespace_remover, string_masker, numbers_masker, sub_tokenizer],
        )
    ...
    def process(self, batch, process_identifier):
        ...
        if all([not accepted for accepted in samples_filter]):
            raise PreprocessingException(process_identifier, "No snippets left after filtering step. Skipping batch")

        assert all(
            [len(l) == len(code_snippets) for l in [func_names, docstrings, stripped_code_snippets, tokens_batch]]
        ), "Intermediate lists differ in length"
        # Select samples that passed the tokens limiter filter
        func_names, docstrings, stripped_code_snippets, tokens_batch = zip(
            *compress(zip(func_names, docstrings, stripped_code_snippets, tokens_batch), samples_filter)
        )
        ...
        if self.language == "java":
            ...
            ast_batch, idx_successful = semantic_parse(
                self.language,
                "--fail-on-parse-error",
                "--json-graph",
                process_identifier,
        ...
        for tokens, ast_graph in zip(tokens_batch, ast_graph_batch):
            token_mapping = {
                i: ast_graph.find_smallest_encompassing_interval(token.source_span) for i, token in enumerate(tokens)
            }
        ...
        assert all([len(batch_part) == num_samples for batch_part in batch_parts]), (
            "Batch parts do not have same " "length"
        )
...
class CTStage1Sample:
    ...
    def remove_punctuation(self):
        ...
        idx = [
            ...
            if len(token.sub_tokens) > 1
            or (any([c.isalpha() for c in token.sub_tokens[0]]) and not token.sub_tokens[0] in {"[INDENT]", "[DEDENT]"})
        ]
