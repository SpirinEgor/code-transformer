...
class CTStage2Sample:
    def __init__(
        ...
        graph_sample: dict,
        token_mapping: dict,
        stripped_code_snippet: str,
        func_name: str,
        docstring: str,
        encoded_func_name: List = None,
    ):
        self.tokens = tokens
    ...
    def from_compressed(compressed_sample):
        ...
        return CTStage2Sample(
            compressed_sample.tokens,
            graph_sample,
            compressed_sample.token_mapping,
            compressed_sample.stripped_code_snippet,
            compressed_sample.func_name,
            compressed_sample.docstring,
            compressed_sample.encoded_func_name if hasattr(compressed_sample, "encoded_func_name") else None,
        )
...
class CTStage2MultiLanguageSample(CTStage2Sample):
    def __init__(
        ...
        super().__init__(
            tokens, graph_sample, token_mapping, stripped_code_snippet, func_name, docstring, encoded_func_name
        )
    ...
    def from_compressed(compressed_sample):
        ...
        for i, distance in enumerate(graph_sample["distances"]):
            if isinstance(distance[0], tuple):
                graph_sample["distances"][i] = (tuple_to_tensor(distance[0]).to_dense(), distance[1], distance[2])
        return CTStage2MultiLanguageSample(
            compressed_sample.tokens,
            graph_sample,
            compressed_sample.token_mapping,
            compressed_sample.stripped_code_snippet,
