...
class CTBaseDataset(__base_dataset):
    ...
    def transform_sample(self, sample: Union[CTStage2Sample, CTStage2MultiLanguageSample]):
        ...
        sequence = torch.tensor(
            [pad_list(token.sub_tokens, self.num_sub_tokens, self.sub_token_pad_value) for token in sample.tokens]
        )
        ...
        assert (
            len({dist_matrix.shape[0] for dist_matrix in distance_matrices}) == 1
        ), "Distance matrices have differing lengths"
                    ...
                    if subtoken == self.unk_id:
                        if hasattr(token, "original_sub_tokens"):
                            ...
                            original_subtoken = split_identifier_into_parts(
                                token.source_span.substring(sample.stripped_code_snippet)
                            )[idx_subtoken]
        ...
        return CTBaseSample(
            ...
            binning_vectors=binning_vectors,
    ...
    def collate_fn(self, batch: List[CTBaseSample]):
        ...
        for sample in batch:
            ...
            for i, (dist_matrix, binning_vector) in enumerate(zip(distance_matrices, binning_vectors)):
                padded_dist_matrix = F.pad(
                    dist_matrix, [0, pad_length, 0, pad_length]
                )  # pad distance matrices with 0 bin
        ...
        if self.use_pointer_network:
            ...
            pointer_pad_mask = pad_sequence(
                [sample.pointer_pad_mask for sample in batch], batch_first=True, padding_value=False
            )
        ...
        return CTBaseBatch(
            tokens=seq_tensors,
            token_types=token_type_tensors,
            node_types=torch.stack(node_type_tensors),
            relative_distances=list(zip(relative_distances.values(), binning_vector_tensors.values())),
            distance_names=distance_names,
            sequence_lengths=seq_lengths,
            pad_mask=padding_mask,
            max_distance_mask=max_distance_masks,
            extended_vocabulary_ids=extended_vocabulary_ids,
            pointer_pad_mask=pointer_pad_mask,
            extended_vocabulary=extended_vocabulary,
            languages=languages,
