...
from code_transformer.modeling.modelmanager import CodeTransformerModelManager, XLNetModelManager, GreatModelManager
...
from code_transformer.preprocessing.dataset.code_summarization import (
    ...
    CTCodeSummarizationDatasetEdgeTypes,
)
...
parser.add_argument("model", choices=["code_transformer", "xl_net", "great"])
...
def print_results():
    for lang_id, language_name in enumerate(language_mapping):
        if predictions[lang_id] and (not args.filter_language or language_mapping[lang_id] == args.filter_language):
            ...
            scores = compute_rouge(predictions[lang_id], lab, pad_id=word_vocab[PAD_TOKEN], predictions_provided=True)

            pred = torch.stack(best_non_unk_predictions[lang_id])

            micro_prec = micro_precision(
                pred, lab, predictions_provided=True, pad_id=word_vocab[PAD_TOKEN], unk_id=word_vocab[UNKNOWN_TOKEN]
            )
            micro_rec = micro_recall(
                pred, lab, predictions_provided=True, pad_id=word_vocab[PAD_TOKEN], unk_id=word_vocab[UNKNOWN_TOKEN]
            )
...
if __name__ == "__main__":
    ...
    for i, batch in tqdm(enumerate(dataloader), total=int(data_manager.approximate_total_samples() / BATCH_SIZE)):
        ...
        batch_non_unk_predictions = get_best_non_unk_predictions(
            output.logits.detach().cpu(), unk_id=word_vocab[UNKNOWN_TOKEN]
        )
        ...
        for prediction, non_unk_prediction, label, lang_id in zip(
            batch_predictions, batch_non_unk_predictions, batch_labels, batch.languages.cpu()
        ):
