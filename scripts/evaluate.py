...
if __name__ == "__main__":
    ...
    data_manager = CTBufferedDataManager(
        DATA_PATH_STAGE_2, config["data_setup"]["language"], partition=args.partition, shuffle=False
    )
    ...
    micro_prec = micro_precision(pred, labels, predictions_provided=True, pad_id=pad_id, unk_id=unk_id)
    micro_rec = micro_recall(pred, labels, predictions_provided=True, pad_id=pad_id, unk_id=unk_id)
    micro_f1 = micro_f1_score(pred, labels, predictions_provided=True, pad_id=pad_id, unk_id=unk_id)
