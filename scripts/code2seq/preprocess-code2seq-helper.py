            ...
            if attention_per_context is not None:
                ...
                for predicted_word, attention_timestep in word_attention_pairs:
                    ...
                    for context, attention in [
                        ...
                        for key in sorted(attention_timestep, key=attention_timestep.get, reverse=True)
                    ][:topk]:
...
def context_full_found(context_parts, word_to_count, path_to_count):
    return context_parts[0] in word_to_count and context_parts[1] in path_to_count and context_parts[2] in word_to_count
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-trd", "--train_data", dest="train_data_path", help="path to training data file", required=True
    )
    parser.add_argument("-ted", "--test_data", dest="test_data_path", help="path to test data file", required=True)
    parser.add_argument("-vd", "--val_data", dest="val_data_path", help="path to validation data file", required=True)
    parser.add_argument(
        "-mc",
        "--max_contexts",
        dest="max_contexts",
    ...
    parser.add_argument(
        "-nh", "--node_histogram", dest="node_histogram", help="node_histogram file", metavar="FILE", required=True
    )
    parser.add_argument(
        "-th",
        "--target_histogram",
    ...
    subtoken_to_count = Common.load_histogram(subtoken_histogram_path, max_size=int(args.subtoken_vocab_size))
    node_to_count = Common.load_histogram(node_histogram_path, max_size=None)
    target_to_count = Common.load_histogram(args.target_histogram, max_size=int(args.target_vocab_size))
