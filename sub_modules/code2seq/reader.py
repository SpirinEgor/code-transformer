...
class Reader:
    ...
    def __init__(
        self, subtoken_to_index, target_to_index, node_to_index, config, is_evaluating=False, num_training_samples=None
    ):
        self.config = config
        ...
        if self.use_multilanguage:
            ...
            for lang in self.languages:
                eval_dataset = "val" if ".val" in Path(config.TEST_PATH).stem else "test"

                self.file_path[lang] = (
                    str(Path(config.TEST_PATH).with_name(lang)) + "." + eval_dataset + ".c2s"
                    if is_evaluating
                    else (str(Path(config.TRAIN_PATH).with_name(lang)) + ".train.c2s")
                )

        else:
            self.file_path = config.TEST_PATH if is_evaluating else (config.TRAIN_PATH + ".train.c2s")
        if self.use_multilanguage:
            for file_path in self.file_path.values():
                if file_path is not None and not os.path.exists(file_path):
                    print(
                        "%s cannot find file: %s"
                        % ("Evaluation reader" if is_evaluating else "Train reader", file_path)
                    )
        else:
            if self.file_path is not None and not os.path.exists(self.file_path):
                print(
                    "%s cannot find file: %s"
                    % ("Evaluation reader" if is_evaluating else "Train reader", self.file_path)
                )
    ...
    def process_dataset(self, *row_parts):
        ...
        target_dense_shape = [
            1,
            tf.maximum(tf.to_int64(self.config.MAX_TARGET_PARTS), split_target_labels.dense_shape[1] + 1),
        ]
        sparse_target_labels = tf.sparse.SparseTensor(
            indices=split_target_labels.indices, values=split_target_labels.values, dense_shape=target_dense_shape
        )
        dense_target_label = tf.reshape(
            tf.sparse.to_dense(sp_input=sparse_target_labels, default_value=Common.PAD), [-1]
        )
        ...
        sparse_split_source = tf.sparse.SparseTensor(
            ...
            dense_shape=[
                ...
                tf.maximum(tf.to_int64(self.config.MAX_NAME_PARTS), split_source.dense_shape[1]),
            ],
        )
        ...
        sparse_split_target = tf.sparse.SparseTensor(
            ...
            dense_shape=[
                self.config.MAX_CONTEXTS,
                tf.maximum(tf.to_int64(self.config.MAX_NAME_PARTS), split_target.dense_shape[1]),
            ],
        )
    ...
    def compute_output(self):
        if self.use_multilanguage:
            dataset = None
            for lang, file_path in self.file_path.items():
                single_dataset = tf.data.experimental.CsvDataset(
                    file_path,
                ...
                lang_ds = tf.data.Dataset.from_tensors(self.languages.index(lang))
                lang_ds = lang_ds.repeat(self.num_training_samples[lang])
                single_dataset = tf.data.Dataset.zip((lang_ds, single_dataset))
                dataset = single_dataset if dataset is None else dataset.concatenate(single_dataset)
        else:
...
if __name__ == "__main__":
    target_word_to_index = {Common.PAD: 0, Common.UNK: 1, Common.SOS: 2, "a": 3, "b": 4, "c": 5, "d": 6, "t": 7}
