...
from code_transformer.modeling.great_transformer.transformer import (
...
class TestGreat(unittest.TestCase):
    def _create_great_model(self, data_manager, use_pointer_network=True, output_subtokens_per_token=6):
        ...
        transformer_config = dict(embed_dim=32, num_layers=3, num_heads=8, ff_dim=64, dropout_rate=0)
        encoder_config = dict(vocab_size=len(word_vocab), num_node_types=len(node_type_vocab))
        decoder_config = dict(
